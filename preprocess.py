...
def clean_text(text):
    """Bulletproof method to completely strip out invalid UTF-8 surrogate characters."""
...
def process_posts(raw_file_path, processed_file_path=None):
    with open(raw_file_path, encoding='utf-8') as file:
        ...
        enriched_posts = []
        for post in posts:
            # Clean all text fields in the post dictionary to prevent any surrogate errors
            cleaned_post_text = clean_text(post.get('text', ''))
            post['text'] = cleaned_post_text
...
def get_unified_tags(posts_with_metadata):
    ...
    for post in posts_with_metadata:
        unique_tags.update(post.get('tags', []))  # Add the tags to the set
